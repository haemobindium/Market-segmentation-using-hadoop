...
public class KMeans {
	...
	public static List<Double> mCenters1 = new ArrayList<Double>();
	...
	public static class Map extends MapReduceBase implements
		...
		public void configure(JobConf job) {
			try {
				...
				if (cacheFiles != null && cacheFiles.length > 0) {
					...
					BufferedReader cacheReader = new BufferedReader(new FileReader("./centroid.txt"));
					try {
						...
						while ((line = cacheReader.readLine()) != null) {
							...
							mCenters1.add(Double.parseDouble(temp[1]));
		...
		public void map(LongWritable key, Text value,
				OutputCollector<DoubleWritable, TextWritable> output,
				Reporter reporter) throws IOException {
			String line = value.toString();
			//**************** split line by deliminator into two points
			//**************** double point = Double.parseDouble(line[k]);
			//**************** double point1 = Double.parseDouble(line[l]);
			
			double point = Double.parseDouble(line);
			
			// System.out.println(point);  will print input data
	...
	public static void run(String[] args) throws Exception {
		...
		while (isdone == false) {
			...
			conf.setMapOutputValueClass(TextWritable.class);
			...
			while (line != null) {
				...
				//System.out.println(c);         will print  centroids after final iteration
				centers_next.add(c);
				line = br.readLine();
			}
			br.close();
